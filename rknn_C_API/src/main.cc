...
static unsigned char *load_model(const char *filename, int *model_size)
{
  ...
  // 指针不变，是FILE *fp结构内的文件光标移动
  // 将文件光标移动到文件末尾
  fseek(fp, 0, SEEK_END);
  // ftell: 返回当前光标离文件开头的偏移量，获取文件大小
  int size = ftell(fp);
...
int main(int argc, char **argv)
{
  ...
  // 加载模型
  // 网络模型已字节的形式存储在model_data中
  unsigned char *model_data = load_model(model_name, &model_data_size);
  // ctx: rknn_context上下文，会在init之后返回为一个有效句柄
  // 加载NPU驱动，将模型加载进NPU中
  ...
  /**********获取输出维度的通道数、高、宽********/
  // input_attrs[0] = [1, 3, 640, 640] or [1, 640, 640, 3]；NCHW或NHWC格式
  int channel = 3;
  int width = 0;
  int height = 0;
  ...
  if (img_width != width || img_height != height)
  {
    ...
    if (option == "resize")
    {
      ...
      // src.virt_addr = img.addr
      // dst.virt_addr = resized_img.addr
      // RGA是rk板子上的硬件加速器
  ...
  // rknn句柄；输入数据的tensor数量；输入的rknn_input
  rknn_inputs_set(ctx, io_num.n_input, inputs);

  rknn_output outputs[io_num.n_output];
  memset(outputs, 0, sizeof(outputs));
  // 不要做float量化，直接输出int8数据
  for (int i = 0; i < io_num.n_output; i++)
  {
