                   ...
                   int32_t zp, float scale)
{
  ...
  // 模型输出 logits（不含 sigmoid），阈值需转为 logit 空间
  float logit_thresh = unsigmoid(threshold);
  int8_t thres_i8 = qnt_f32_to_affine(logit_thresh, zp, scale);
  for (int a = 0; a < 3; a++)
  {
    for (int i = 0; i < grid_h; i++)
    {
      for (int j = 0; j < grid_w; j++)
      {
        ...
        if (box_confidence >= thres_i8)
        {
          ...
          // 反量化 + sigmoid 得到概率
          float tx = sigmoid(deqnt_affine_to_f32(*in_ptr, zp, scale));
          float ty = sigmoid(deqnt_affine_to_f32(in_ptr[grid_len], zp, scale));
          float tw = sigmoid(deqnt_affine_to_f32(in_ptr[2 * grid_len], zp, scale));
          float th = sigmoid(deqnt_affine_to_f32(in_ptr[3 * grid_len], zp, scale));

          float box_x = tx * 2.0 - 0.5;
          float box_y = ty * 2.0 - 0.5;
          float box_w = tw * 2.0;
          float box_h = th * 2.0;
          ...
          {
            float obj_conf = sigmoid(deqnt_affine_to_f32(box_confidence, zp, scale));
            float cls_conf = sigmoid(deqnt_affine_to_f32(maxClassProbs, zp, scale));
            objProbs.push_back(obj_conf * cls_conf);
